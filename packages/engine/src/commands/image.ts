...
import { hasPlane, walkLayers, type Doc, type Layer, type SmartObjectLayer, type VectorLayer } from '../document.js';
import { retransformVector } from '../type-layers.js';
 ...
 */
export function mapLayers(layers: readonly Layer[], fn: (p: Plane) => Plane, smart?: (l: SmartObjectLayer) => Layer, shape?: (l: VectorLayer) => Layer): Layer[] {
  return layers.map((l) => {
    if (l.kind === 'smart' && smart) return smart(l);
    if ((l.kind === 'shape' || l.kind === 'type') && shape) return shape(l);
...
export function transformVectorMasks(layers: readonly Layer[], matrix: Mat): Layer[] {
  return layers.map((l) => {
    const vm = l.vectorMask ? { vectorMask: { ...l.vectorMask, path: transformPath(l.vectorMask.path, matrix) } } : {};
...
function mapGeometry(layers: readonly Layer[], fn: (p: Plane) => Plane, matrix: Mat, size: { width: number; height: number }): Layer[] {
  return transformVectorMasks(mapLayers(layers, fn, (l) => retransform(l, matrix, size, fn), (l) => retransformVector(l, matrix, size, fn)), matrix);
}
...
export function revealAll(doc: Doc): Doc {
  ...
  for (const { layer } of walkLayers(doc.layers)) {
    if (hasPlane(layer) && layer.kind !== 'smart') bounds = rectUnion(bounds, tightBounds(layer.plane.base));
  }
  const dx = -bounds.x0;
  const dy = -bounds.y0;
  return {
